...
namespace Template.Service.DAO
{
    public static class EvaluacionService
    {
        ...
        public static bool AssignEmpleados(int Id, string Empleados)
        {
            ...
            using (var Dc = new TemplateEntities())
            {
                ...
                foreach (var Empleados_ in empleadosIds)
                {
                    ...
                    {
                        ///////////////////////////////////////////////////////////
                        idEmpleadoEvaluado = Convert.ToInt32(Empleados_),
                        idEvaluacion = Id
                    };
        ...
        public static List<Usuario> RegistrarEvaluacion(int Id, string Empleados)
        {
            var empleadosIds = Empleados.Split(',');
            ...
            using (var Dc = new TemplateEntities())
            {
                ...
                //if (!empleadosIds.Any()) return new List<Usuario>();
                var Emp_Eval_ = new List<Emp_Eval>();
                var UsuariosCorreo = new List<Usuario>();

                var Eval = FindById(Id); //'Dc.Evaluacion.Where(x => x.id == Id);
                                
                switch (Eval.tipoEval)
                {
                    case 1: // Evaluacion 90 grados Evaluan los jefe
                        foreach (var Empleados_ in empleadosIds)
                        {
                            if (string.IsNullOrEmpty(Empleados_)) continue;

                           
                            var empleadoJ = EmpleadoService.ObtenerJefatura(Convert.ToInt32(Empleados_));
                            if (empleadoJ == null) continue; 
                            var EmpEval = new Emp_Eval
                            {
                                idEmpleadoEvaluador = empleadoJ.id,
                                idEmpleadoEvaluado = Convert.ToInt32(Empleados_),
                                idEvaluacion = Id,
                                fechaCreacion = DateTime.Now,
                                fechaModificacion = DateTime.Now,
                                activo = true,
                                comentario = empleadoJ.id_Usuario.ToString()
                            };
                            
                            Emp_Eval_.Add(EmpEval);

                            var evaluadorCorreo = UserService.FindById(Convert.ToInt32(Empleados_));
                            UsuariosCorreo.Add(evaluadorCorreo);
                           }
